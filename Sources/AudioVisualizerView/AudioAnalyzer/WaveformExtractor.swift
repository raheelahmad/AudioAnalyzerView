    ...
    fileprivate static func _extract( samplesFrom reader: AVAssetReader,
        ...
        asset.loadValuesAsynchronously(forKeys: ["duration"]) {
            ...
            case .loaded:
                ...
                let assetDurationSeconds = duration / timscale
                // Asset is sampled many times per second. So calculate total number of samples
                let numOfTotalSamples = (asbd.pointee.mSampleRate) * assetDurationSeconds
                ...
                // How many samples we need per pixel (for a display). So, performs bucketing.
                let samplesPerPixel = Int(max(1,  Double(channelCount) * numOfTotalSamples / Double(desiredNumberOfSamples)))
//                samplesPerPixel = min(32000, samplesPerPixel)
...
//                    CMBlockBufferGetDataPointer(readBuffer, 0, &readBufferLength, nil, &readBufferPointer)
                    CMBlockBufferGetDataPointer(readBuffer, atOffset: 0, lengthAtOffsetOut: nil, totalLengthOut: &readBufferLength, dataPointerOut: &readBufferPointer)
